...
> **See also:** [README](README.md) for installation and quick start, [Configuration Reference](docs/configuration-reference.md) for all script attributes, [Consent Banner](docs/consent-banner.md) for cookie consent setup, [GA4 Ecommerce](docs/ga4-ecommerce.md) for ecommerce tracking.
...
Before you’re doing anything, make sure the AS Tracking Code script is loaded in your project. See the [README](README.md#quick-start) for installation steps.
...
Static events are any events that is triggered by any of the clickable elements with static properties. **The event from section 2.2 will be sent to your analytics platform(s) like this:**

**PostHog:**
```js
posthog.capture('Name of Your Event', {
...
**GA4** (spaces in event names are automatically converted to underscores):
```js
gtag('event', 'Name_of_Your_Event', {
  name_of_the_property: 'value_of_the_property',
  name_of_another_property: 'value_of_another_property',
  name_of_yet_another_property: 'value_of_yet_another_property',
});
...
2. When the user loads the Blog Post, the Blog Post Viewed event will be triggered:

**PostHog:**
```jsx
posthog.capture('Blog Post Viewed', {
...
When the button is clicked, the event is sent to your analytics platform(s):

**PostHog:**
```jsx
posthog.capture('Featured Product Clicked', {
  blog_post_title: 'This is a blog post title.',
  blog_post_duration: 5,
});
```
...
**GA4:**
```jsx
gtag('event', 'Featured_Product_Clicked', {
  blog_post_title: 'This is a blog post title.',
  blog_post_duration: 5,
});
```
...
The events and identity data are sent to your analytics platform(s):
...
**GA4** (the `email` field is used as `user_id` by default; other identify fields become user properties):
```jsx
...
gtag('set', { user_id: 'john@acme.com' });
...
The final event call to your analytics platform(s):
