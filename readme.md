...
| `data-customerio-auto-pageview="true"`  | Fire `cioanalytics.page()` on pageviews from the tracker. Only enable if you remove the snippet's own `cioanalytics.page()` call | `false` |
...
| [Connecting Customer.io](docs/connecting-customerio.md)    | Customer.io (`cioanalytics`) setup, identify/track/page behavior, pageview note |
